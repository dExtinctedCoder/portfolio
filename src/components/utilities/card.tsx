...
export const PortfolioCard = ({title, detail, href, bg}: portfolioPropsType) => {
  return (
    ...
    >
      ...
      <div className="w-full h-full flex flex-col justify-center items-center p-8 px-10 z-10 text-white bg-black hover:bg-[rgba(22,22,22,0.55)] dark:bg-[#171616] dark:hover:bg-[rgba(22,22,22,0.55)]">
        <h5 className="text-2xl font-bold text-green-light mb-2">{title}</h5>
        <p className="mb-5 text-xl font-semibold">{detail}</p>
        <Link 
          to={href}
          target="blank"
          className="bg-gold-light text-black border-4 border-gold-light py-1 px-4 flex items-center rounded-lg hover:bg-black hover:text-gold-light hover:border-black"
